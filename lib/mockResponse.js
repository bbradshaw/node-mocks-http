...
function createResponse(options = {}) {
    ...
    mockResponse.writeHead = function writeHead(statusCode, statusMessage, headers) {
        ...
        if (this.headersSent) {
            const headersSent = new Error('Cannot set headers after they are sent to the client');
            headersSent.code = 'ERR_HTTP_HEADERS_SENT';
            throw headersSent;
        }
    ...
    mockResponse.setHeader = function setHeader(name, value) {
        if (mockResponse.headersSent) {
            const headersSentErr = new Error('Cannot set headers after they are sent to the client');
            headersSentErr.code = 'ERR_HTTP_HEADERS_SENT';
            throw headersSentErr;
        }
